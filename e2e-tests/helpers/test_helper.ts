...
export class PageObject {
  ...
  async expectMobileDevelopmentSection() {
    await expect(this.page.getByText("Mobile Development")).toBeVisible({
      timeout: Timeout.MEDIUM,
    });
  }

  async expectCapacitorControls() {
    await expect(this.page.getByText("Capacitor")).toBeVisible();
    await expect(this.page.getByRole("button", { name: /Sync.*Android/i })).toBeVisible();
    await expect(this.page.getByRole("button", { name: /Sync.*iOS/i })).toBeVisible();
  }

  async expectFlutterControls() {
    await expect(this.page.getByText("Flutter")).toBeVisible();
    await expect(this.page.getByRole("button", { name: /Sync.*Android/i })).toBeVisible();
    await expect(this.page.getByRole("button", { name: /Sync.*iOS/i })).toBeVisible();
  }

  async expectSourceCodeSection() {
    await expect(this.page.getByText("Source code")).toBeVisible();
    await expect(this.page.getByRole("button", { name: "Open in folder" })).toBeVisible();
    await expect(this.page.getByRole("button", { name: "Copy path" })).toBeVisible();
  }
...
export async function expectNoConsoleErrors(page: Page, errorKeywords: string[] = []) {
  ...
  // Wait a moment for any pending console messages
  await page.waitForTimeout(1000);

  // Check for specific error keywords
  const relevantErrors = messages.filter(message => 
    errorKeywords.some(keyword => message.includes(keyword))
  );

  if (relevantErrors.length > 0) {
    throw new Error(`Found console errors containing keywords ${errorKeywords.join(', ')}:\n${relevantErrors.join('\n')}`);
  }
}
