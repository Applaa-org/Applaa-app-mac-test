    ...
    <div className="flex flex-col items-center justify-center max-w-3xl m-auto p-8">
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col gap-4 mt-4">
          {selectedIdea && (
            <div className="text-center text-sm text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-4 py-2 rounded-lg border border-blue-200 dark:border-blue-800">
              💡 <strong>{selectedIdea.label}</strong> prompt loaded! Switch between Web/Mobile to see different versions.
            </div>
          )}
          <div className="flex flex-wrap gap-4 justify-center">
            {randomPrompts.map((item, index) => {
              // Use enhanced prompt based on selected platform
              const getEnhancedPrompt = () => {
                const isExpoSelected = settings?.selectedPlatform === 'expo';
                return isExpoSelected ? item.mobilePrompt : item.webPrompt;
              };

              const handleIdeaClick = () => {
                setSelectedIdea(item); // Store the selected idea for platform switching
                setInputValue(getEnhancedPrompt());
              };

              const isSelected = selectedIdea?.label === item.label;
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-4 py-2 rounded-xl border transition-all duration-200 active:scale-[0.98] ${
                    isSelected
                      ? "border-blue-300 bg-blue-50 shadow-md dark:border-blue-600 dark:bg-blue-900/20"
                      : "border-gray-200 bg-white/50 backdrop-blur-sm hover:bg-white hover:shadow-md hover:border-gray-300 dark:bg-gray-800/50 dark:border-gray-700 dark:hover:bg-gray-800 dark:hover:border-gray-600"
                  }`}
                >
                  <span className={`${isSelected ? "text-blue-600 dark:text-blue-400" : "text-gray-700 dark:text-gray-300"}`}>
                    {item.icon}
                  </span>
                  ...
                  {isSelected && (
                    <span className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded-full dark:bg-blue-800 dark:text-blue-300">
                      Active
                    </span>
                  )}
                </button>
          ...
          <button
            ...
            onClick={() => {
              setRandomPrompts(getRandomPrompts());
              setSelectedIdea(null); // Clear selected idea when refreshing prompts
            }}
