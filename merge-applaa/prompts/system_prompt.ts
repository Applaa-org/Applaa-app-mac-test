  ...
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        const expoPackages = ['expo', 'react-native', '@expo/cli', 'expo-router'];
        const hasExpoPackages = expoPackages.some(pkg => dependencies[pkg]);
        
        if (hasExpoPackages) {
          return true;
        }
      } catch (error) {
        logger.error('Error reading package.json for Expo detection:', error);
      }
    }
    ...
    if (fs.existsSync(appDirPath)) {
      // Check for typical Expo Router files
      const expoRouterFiles = ['_layout.tsx', '_layout.js', 'index.tsx', 'index.js'];
      const hasExpoRouterFiles = expoRouterFiles.some(file => 
        fs.existsSync(path.join(appDirPath, file))
      );
      if (hasExpoRouterFiles) {
        return true;
      }
    }
    ...
    logger.error('Error detecting Expo app:', error);
    return false;
  }
}
