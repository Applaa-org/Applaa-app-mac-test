...
export function CapacitorControls({ appId }: CapacitorControlsProps) {
  ...
  const { data: isCapacitor, isLoading, refetch } = useQuery({
    queryKey: ["is-capacitor", appId, "v2"],
  ...
  // Fetch app path info for source code display
  const { data: appBase } = useQuery({
    queryKey: ["apps-base-path"],
    queryFn: () => IpcClient.getInstance().getAppsBasePath(),
  });
  ...
  const fullAppPath = appBase?.basePath && appInfo?.path
    ? `${appBase.basePath}\\${appInfo.path}`
    : undefined;
  const iosPath = fullAppPath ? `${fullAppPath}\\ios` : undefined;
  const androidPath = fullAppPath ? `${fullAppPath}\\android` : undefined;
        ...
        <CardContent>
          ...
          {fullAppPath && (
            <div className="mt-3 border-t pt-3">
              <div className="text-xs font-medium mb-2">Source code</div>
              ...
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    variant="outline"
                    className="h-7 px-2"
                    onClick={() => iosPath && IpcClient.getInstance().showItemInFolder(iosPath)}
                  >
                    Open
                  </Button>
                  <Button
                    ...
                    variant="ghost"
                    className="h-7 px-2"
                    onClick={() => iosPath && navigator.clipboard.writeText(iosPath)}
                  >
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    variant="outline"
                    className="h-7 px-2"
                    onClick={() => androidPath && IpcClient.getInstance().showItemInFolder(androidPath)}
                  >
                    Open
                  </Button>
                  <Button
                    ...
                    variant="ghost"
                    className="h-7 px-2"
                    onClick={() => androidPath && navigator.clipboard.writeText(androidPath)}
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
