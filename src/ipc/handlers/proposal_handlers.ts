  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      try {
        ...
        logger.warn("Error processing proposal for chatId", chatId + ":", err);
        // Fallback without app relation for legacy DB compatibility
        chat = await db.query.chats.findFirst({
          where: eq(chats.id, chatId),
          with: {
            messages: {
        ...
        if (chat) {
          // Manually get app with legacy fallback
