...
async function getAppSafe(appId: number): Promise<any> {
  try {
    ...
    logger.warn(
      "version_handlers.getAppSafe: falling back to legacy SELECT due to:",
      err,
    );
    const row = db.$client
      .prepare(
...
export function registerVersionHandlers() {
  handle("list-versions", async (_, { appId }: { appId: number }) => {
    ...
    try {
      ...
      logger.warn("list-versions: fallback app select due to:", err);
      const row = db.$client
        .prepare(
          "SELECT id, name, path, created_at as createdAt, " +
            "github_org as githubOrg, github_repo as githubRepo, github_branch as githubBranch, " +
  ...
  handle(
    ...
    async (_, { appId }: { appId: number }): Promise<BranchResult> => {
      ...
      if (!fs.existsSync(path.join(appPath, ".git"))) {
        logger.log(`Initializing git repository for app ${appId} at ${appPath}`);
        try {
          // Initialize git repo
          await git.init({
            fs,
          ...
          logger.log(`Successfully initialized git repository for app ${appId}`);
        } catch (error) {
          logger.error(`Failed to initialize git repository for app ${appId}:`, error);
