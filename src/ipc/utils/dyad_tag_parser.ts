...
export function getDyadWriteTags(fullResponse: string): {
  ...
  // Support both dyad-write and applaa-write tags
  const dyadWriteRegex = /<(?:dyad-write|applaa-write)([^>]*)>([\s\S]*?)<\/(?:dyad-write|applaa-write)>/gi;
  ...
  while ((match = dyadWriteRegex.exec(fullResponse)) !== null) {
    ...
    if (pathMatch && pathMatch[1]) {
      ...
      logger.warn(
        "Found <dyad-write> or <applaa-write> tag without a valid 'path' attribute:",
        match[0],
      );
    }
  }
...
export function getDyadDeleteTags(fullResponse: string): string[] {
  const dyadDeleteRegex =
    /<(?:dyad-delete|applaa-delete) path="([^"]+)"[^>]*>([\s\S]*?)<\/(?:dyad-delete|applaa-delete)>/g;
  let match;
  const paths: string[] = [];
