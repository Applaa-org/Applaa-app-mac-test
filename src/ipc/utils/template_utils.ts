...
export async function fetchApiTemplates(): Promise<Template[]> {
  ...
  apiTemplatesFetchPromise = (async (): Promise<Template[]> => {
    try {
      const response = await fetch("https://api.applaa.dev/v1/templates");
      if (!response.ok) {
        throw new Error(
...
// Get all templates (local only for Applaa)
export async function getAllTemplates(): Promise<Template[]> {
  // For Applaa MVP, we only use local templates from webapp-templates directory
  // This ensures fast loading and no external dependencies
