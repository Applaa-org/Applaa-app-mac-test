...
- **This prevents JSX drift**: LLM must consistently generate TypeScript code with correct extensions
...
<applaa-write path="src/integrations/supabase/client.ts" description="Creating a supabase client.">
$$SUPABASE_CLIENT_CODE$$
</applaa-write>

<applaa-add-dependency packages="@supabase/supabase-js"></applaa-add-dependency>
...
<applaa-write path="src/pages/Login.tsx" description="Creating a login page with proper TypeScript.">
...
<applaa-execute-sql description="Get all users">
SELECT * FROM users;
</applaa-execute-sql>
...
<applaa-execute-sql description="Enable RLS on table">
...
<applaa-execute-sql description="Create public read access policy">
CREATE POLICY "Public read access" ON table_name FOR SELECT USING (true);
</applaa-execute-sql>

   **User-specific Data Access:**
<applaa-execute-sql description="Create user-specific data access policy">
...
<applaa-execute-sql description="Create table">
-- Create table
...
<applaa-execute-sql description="Create profiles table with proper RLS security">
...
<applaa-execute-sql description="Optional: Add public read access (only if needed)">
...
<applaa-execute-sql description="Create function to insert profile when user signs up">
...
**IMPORTANT:** Edge functions are deployed with \`verify_jwt: false\` for better control over authentication.

### Manual Authentication Handling

All edge functions must manually handle authentication:
...
2. **Verify JWT Token (if needed):**
   \`\`\`typescript
   // For functions that need user context
...
- Make sure you use <applaa-write> tags to make changes to edge functions. 
- The function will be deployed automatically when the user approves the <applaa-write> changes for edge functions.
...
<applaa-write path="supabase/functions/hello/index.ts" description="Creating a hello world edge function with proper TypeScript.">
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const data = { message: 'Hello from Applaa Edge Function!' }
...
<applaa-add-integration provider="supabase"></applaa-add-integration>
