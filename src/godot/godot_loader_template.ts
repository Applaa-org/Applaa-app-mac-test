/**
 * Godot Loader Template Generator
 * Creates the Loader.tscn scene and Loader.gd script that dynamically constructs games
 */
...
const logger = log.scope("godot_loader_template");

/**
 * Generate the Loader scene and script
 * This is the entry point that reads game_spec.json and constructs the game
 */
export async function generateLoaderScene(
  projectPath: string,
  spec: GameSpecification
): Promise<void> {
  // Generate Loader.gd script
  await generateLoaderScript(projectPath, spec);

  // Generate Loader.tscn scene
  await generateLoaderSceneFile(projectPath, spec);
}

/**
 * Generate Loader.gd - The main script that reads JSON and constructs the game
 */
async function generateLoaderScript(
  projectPath: string,
  spec: GameSpecification
): Promise<void> {
  const scriptPath = path.join(projectPath, "Loader.gd");

  const loaderScript = `extends Node
...
`;
  ...
  fs.writeFileSync(scriptPath, loaderScript);
  logger.info("Generated Loader.gd script");
}

/**
 * Generate Loader.tscn - The entry scene file
 */
async function generateLoaderSceneFile(
  projectPath: string,
  spec: GameSpecification
): Promise<void> {
  const scenePath = path.join(projectPath, "Loader.tscn");

  const sceneContent = `[gd_scene load_steps=2 format=3 uid="uid://loader_scene"]
...
`;
  ...
  logger.info("Generated Loader.tscn scene");
}
